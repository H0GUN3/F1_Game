...
1. 문서 구조 전면 재정비 (Added)

기존 변경사항 문서는 시간 순서가 명확하지 않았다.

변경 유형별 구분이 없어 추적이 어려웠다.

이에 따라 변경사항을 기능 / 구조 / 설계 / 개선 단위로 재구성하였다.

각 변경 항목은 독립적으로 이해 가능하도록 작성되었다.

문단 단위로 의미가 끊기도록 문장 구조를 조정하였다.

추상적인 표현보다 실제 코드·로직 기준 설명을 우선하였다.

동일한 맥락의 설명은 하나의 섹션으로 통합하였다.

변경 이유와 결과가 함께 드러나도록 서술 방식을 개선하였다.

향후 추가 변경사항이 자연스럽게 이어질 수 있도록 형식을 고정하였다.

2. 레이스 시스템 설명 보강 (Added)

레이스 진행은 단일 흐름이 아닌 상태 기반으로 이해해야 한다.

레이스 시작 이전, 진행 중, 랩 완료, 종료 상태를 구분하였다.

체크포인트 통과 조건과 랩 판정 기준을 명확히 기술하였다.

잘못된 순서 통과 시 처리 방식에 대한 설명을 추가하였다.

HUD 갱신 시점과 레이스 로직 간의 관계를 정리하였다.

타이머와 랩 카운트 간의 의존성을 명확히 설명하였다.

입력 처리와 레이스 상태의 관계를 분리하여 설명하였다.

확장 시 예상되는 문제점도 함께 기술하였다.

3. 차량 로직 및 밸런스 설명 확장 (Added)

차량 성능은 단일 수치가 아닌 복합 요소의 결과이다.

가속, 감속, 회전, 최대 속도의 상호작용을 정리하였다.

각 수치가 플레이 감각에 미치는 영향을 서술하였다.

트랙 구조에 따른 체감 차이를 설명하였다.

초보자/숙련자 차량 설계 방향을 예시로 제시하였다.

수치 조정 시 발생할 수 있는 부작용을 정리하였다.

테스트 기반 밸런스 조정의 필요성을 강조하였다.

4. 초기 설계 가정 중 불필요해진 설명 정리 (Removed)

반복적으로 등장하던 추상적 평가 문장을 제거하였다.

“중요하다”, “도움이 된다” 등 의미 없는 표현을 삭제하였다.

구조 설명 없이 결과만 언급하던 문장을 제거하였다.

동일한 내용을 다른 표현으로 반복하던 문단을 정리하였다.

초기 단계에서만 유효했던 가정을 삭제하였다.

현재 구조와 맞지 않는 설명을 제거하였다.

5. 중복·불명확 문장 정리 (Removed)

의미가 모호한 주관적 평가 문장을 삭제하였다.

코드와 직접 연결되지 않는 서술을 제거하였다.

앞뒤 문단과 논리적으로 이어지지 않는 문장을 삭제하였다.

다른 항목에서 이미 설명된 내용을 반복한 문단을 제거하였다.

6. 문서 유지보수 관점 개선 (Added)

이후 변경사항 추가 시 참고할 기준을 명시하였다.

변경 단위는 “의미 기준”으로 추가하도록 규칙을 정리하였다.

임시 메모성 문장은 문서에 포함하지 않도록 기준을 세웠다.

변경사항 문서가 단순 로그가 아닌 설계 기록이 되도록 방향을 잡았다.